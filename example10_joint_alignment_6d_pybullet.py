...
def get_joint_keypoints_from_angles_6d(params, opt, cam_K, robotId):
    numJoints = p.getNumJoints(robotId)
    jointAngles = params[:numJoints]
    pose6d = params[numJoints:]
    tx, ty, tz, roll, pitch, yaw = pose6d
...
basePosition_1 = np.array([0, 0, 0])
basePoseQuat_1 = np.array([0, 0, 0, 1]) # quaternion [x,y,z,w]
basePoseEuler_1 = p.getEulerFromQuaternion(basePoseQuat_1) # Euler angle extrinsic rotation order (Rx -> Ry -> Rz)
p.resetBasePositionAndOrientation(robotId, basePosition_1, basePoseQuat_1)
basePosition_2 = basePosition_1
basePoseEuler_2 = basePoseEuler_1
numJoints = p.getNumJoints(robotId)
...
jpnp_params = np.hstack((jointAngles, basePosition_1, basePoseEuler_1, basePosition_2, basePoseEuler_2)) # [18]
numParams = len(jpnp_params) # [18]

eps = 1e-6 # epsilon for Jacobian approximation
# eps = np.linspace(1e-6, 1e-6, num_params)
iterations = 1000 # This value can be adjusted.
for iter in range(iterations):    
    print(f'iter: {iter}, jointAngles: {jointAngles}')
    jpnp_params_1 = jpnp_params[:numJoints+6]
    jpnp_params_2 = np.hstack((jpnp_params[:numJoints], jpnp_params[-6:]))
    # get joint 2d keypoint from 3d points and camera model
    keypoints1 = get_joint_keypoints_from_angles_6d(jpnp_params_1, opt, cam_K, robotId)
    keypoints2 = get_joint_keypoints_from_angles_6d(jpnp_params_2, opt, cam_K, robotId)
    keypoints = np.vstack((keypoints1, keypoints2)).reshape(-1) # [24]
    
    # Jacobian approximation: keypoint rate (변화량)
    Jacobian = np.zeros((numJoints*2*2, numParams)) # [24, 18]
    for col in range(numParams):
        eps_array = np.zeros(numParams)
        eps_array[col] = eps
        jpnp_params_eps = jpnp_params + eps_array
        jpnp_params_1_eps = jpnp_params_eps[:numJoints+6]
        jpnp_params_2_eps = np.hstack((jpnp_params_eps[:numJoints], jpnp_params_eps[-6:]))
        
        keypoints_eps1 = get_joint_keypoints_from_angles_6d(jpnp_params_1_eps, opt, cam_K, robotId)
        keypoints_eps2 = get_joint_keypoints_from_angles_6d(jpnp_params_2_eps, opt, cam_K, robotId)
    ...
    print(f'position_1: {jpnp_params[6:9]}')
    print(f'position_2: {jpnp_params[12:15]}')
