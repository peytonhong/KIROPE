...
class DigitalTwin():
    ...
    def __init__(self, urdf_path="../urdfs/kuka_iiwa/model.urdf", 
        ...
        # lets create a robot
        self.kukaId = p.loadURDF(urdf_path, [0, 0, 0], useFixedBase=True)
        ...
        jointInfo = p.getJointInfo(self.kukaId, 0)
        lower_limit = [p.getJointInfo(self.kukaId, i)[8] for i in range(self.numJoints)]
        upper_limit = [p.getJointInfo(self.kukaId, i)[9] for i in range(self.numJoints)]
        ...
        for link_num in range(len(link_meshes)):    
            if link_num==0: # base
                link_state = p.getBasePositionAndOrientation(bodyUniqueId=self.kukaId)      
                pos_world = link_state[0]
                rot_world = link_state[1]
            else: # link
                link_state = p.getLinkState(bodyUniqueId=self.kukaId, linkIndex=link_num-1)        
    ...
    def forward(self):
        ...
        label_paths = sorted(glob(os.path.join(self.opt.inputf, '*.json')))
        with open(label_paths[self.opt.idx]) as json_file:
        ...
        for iter in range(iterations):    
            ...
            keypoints = self.get_joint_keypoints_from_angles(jointAngles, self.opt, camera_name = 'camera')
            ...
            for col in range(self.numJoints):
                ...
                keypoints_eps = self.get_joint_keypoints_from_angles(jointAngles+eps_array, self.opt, camera_name = 'camera')
            ...
            for j in range(self.numJoints):
                p.resetJointState(bodyUniqueId=self.kukaId,
                                jointIndex=j,
                                targetValue=(jointAngles[j]),
                                )    
            p.stepSimulation()
            link_world_state = self.get_joint_world_position(self.kukaId)   # [num_link, position, rotation]
            ...
            # get_my_keypoints(camera_entity=camera, robotId=kukaId, joint_world_position=joint_world_position, self.opt=self.opt)
            if self.save_images:
                if self.make_joint_sphere:
    ...
    def get_joint_world_position(self, bodyUniqueId):
    ...
    def get_joint_keypoints_from_angles(self, jointAngles, opt, camera_name = 'camera'):
        cam_matrix = nvisii.entity.get(camera_name).get_transform().get_world_to_local_matrix()
        cam_proj_matrix = nvisii.entity.get(camera_name).get_camera().get_projection()
